...
Tombol menu utama ini sekarang menampilkan daftar **nama produk sebagai
tombol** (dikelompokkan per kategori kalau tokomu punya lebih dari satu
kategori). Tap salah satu nama produk akan membuka halaman **deskripsi +
foto khusus produk itu**, dengan dua tombol: **Order Sekarang** (langsung
membuka live chat dengan admin, sudah dilampiri info produk yang
ditanyakan) dan **Kembali** (balik ke daftar katalog).

Atur isinya lewat panel admin:
- **Panel Admin → 📋 Kelola Katalog & Pricelist** — ubah nama tombol menu
  utama dan deskripsi/foto pembuka (opsional, tampil di atas daftar
  produk).
- **Panel Admin → 🗂 Kelola Kategori & Produk → pilih produk** — di sinilah
  kamu atur **deskripsi** dan **foto** khusus tiap produk (yang tampil ke
  customer saat mereka tap nama produk itu), selain harga dan catatan.
...
**di dalam halaman katalog produk** sengaja dibuat lebih simpel: langsung
live chat ke admin, cocok untuk toko yang mau negosiasi/konfirmasi manual
dulu sebelum urusan pembayaran.
