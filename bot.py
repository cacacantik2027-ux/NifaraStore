...
def main():
    ...
    # ── Terima foto bukti dari user biasa ──────────────────────
    # Filter: bukan admin/owner, kirim foto/dokumen
    app.add_handler(MessageHandler(
        filters.PHOTO | filters.Document.IMAGE,
        terima_bukti_pembayaran
    ))

    # ── Terima foto QRIS dari owner ───────────────────────────
    # (Handler di atas sudah cukup — kita pisah via context.user_data)
    # Supaya handler owner_upload_qris tidak bentrok, kita overide:
    # Handler foto diproses oleh satu handler, cek user_data di dalamnya.
    # Karena fungsi terima_bukti_pembayaran sudah menjadi fallback,
    # tambahkan handler khusus owner yang diletakkan SEBELUMNYA:
    ...
    # Hapus handler foto sebelumnya dan pasang dispatcher
    app.handlers[0] = []   # reset group 0
    for h in [
        CommandHandler("start",        start),
        CommandHandler("konfirmasi",   cmd_konfirmasi),
        CommandHandler("tambah_admin", cmd_tambah_admin),
        CommandHandler("hapus_admin",  cmd_hapus_admin),
        CommandHandler("broadcast",    cmd_broadcast),
        CommandHandler("admin",        admin_menu),
        CommandHandler("owner",        owner_menu),
        CallbackQueryHandler(callback_router),
        MessageHandler(filters.PHOTO | filters.Document.IMAGE, foto_dispatcher),
    ]:
        app.add_handler(h)
    ...
    print("🤖 Bot Store (PostgreSQL + QRIS Upload + Auto-Expire) berjalan...")
