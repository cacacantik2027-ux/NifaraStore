...
// Halaman "Katalog & Pricelist": daftar NAMA PRODUK sebagai tombol
// (dikelompokkan per kategori kalau lebih dari 1 kategori). Tap salah satu
// produk -> masuk ke productInfoKeyboard() (deskripsi + foto produk itu).
function catalogListKeyboard() {
  const { categories } = db.get();
  const catsWithProducts = categories.filter((c) => c.products.length > 0);
  const showHeaders = catsWithProducts.length > 1;
  const rows = [];
  catsWithProducts.forEach((cat) => {
    if (showHeaders) {
      // Baris judul kategori, tidak bisa di-tap (callback_data "noop").
      rows.push([btn(`— ${cat.name} —`, { callback_data: "noop" })]);
    }
    cat.products.forEach((p) => {
      rows.push([
        btn(`${p.name} — ${db.formatRupiah(p.price)}`, { callback_data: `prodinfo:${cat.id}:${p.id}` }),
      ]);
    });
  });
...
// Halaman detail 1 produk: deskripsi + foto (diatur admin per-produk),
// dengan tombol Order Sekarang (langsung buka live chat dengan admin
// terkait produk ini) dan Kembali (ke daftar katalog).
function productInfoKeyboard(catId, prodId) {
  return {
    inline_keyboard: [
      [btn("🛒 Order Sekarang", { callback_data: `orderlive:${catId}:${prodId}` }, "success")],
