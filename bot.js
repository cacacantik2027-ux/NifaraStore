...
bot.on("callback_query", async (q) => {
  ...
  try {
    // Tombol judul kategori di daftar katalog (non-aktif, sekadar pemisah)
    if (data === "noop") return;
    ...
    if (data.startsWith("prodinfo:")) {
      const [, catId, prodId] = data.split(":");
      return cust.showProductInfo(bot, chatId, messageId, catId, prodId);
    }
    if (data.startsWith("orderlive:")) {
      const [, catId, prodId] = data.split(":");
      return cust.startLiveChatOrder(bot, chatId, messageId, catId, prodId, q.from);
    }
    ...
    if (data.startsWith("admin:")) {
      ...
      if (action === "prod_desc") return adm.askProductDescription(bot, chatId, userId, messageId, parts[2], parts[3]);
      if (action === "prod_img") return adm.askProductPhoto(bot, chatId, userId, messageId, parts[2], parts[3]);
      if (action === "prod_imgdel") { adm.deleteProductPhoto(parts[2], parts[3]); return adm.showProductDetailAdmin(bot, chatId, messageId, parts[2], parts[3]); }
...
bot.on("message", async (msg) => {
  ...
  if (session.step === "await_product_description" && msg.text) {
    const { catId, prodId } = session;
    adm.updateProductDescription(catId, prodId, msg.text);
    adm.clearSession(userId);
    return bot.sendMessage(chatId, "✅ Deskripsi produk berhasil diperbarui.", {
      reply_markup: { inline_keyboard: [kb.backButton(`admin:prod:${catId}:${prodId}`)] },
    });
  }

  if (session.step === "await_product_photo" && msg.photo) {
    const { catId, prodId } = session;
    const fileId = msg.photo[msg.photo.length - 1].file_id;
    adm.setProductPhoto(catId, prodId, fileId);
    adm.clearSession(userId);
    return bot.sendMessage(chatId, "✅ Foto produk berhasil disimpan.", {
      reply_markup: { inline_keyboard: [kb.backButton(`admin:prod:${catId}:${prodId}`)] },
    });
  }
