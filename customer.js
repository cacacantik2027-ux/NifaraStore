...
// admin lewat Panel Admin → Kelola Katalog & Pricelist) lalu daftar tombol
// NAMA PRODUK saja (tanpa tombol harga). Tap salah satu nama produk untuk
// lihat halaman detailnya (lihat showCatalogProductDetail di bawah).
async function showCatalogList(bot, chatId, messageId) {
  ...
  // Hapus pesan menu lama, kirim pesan baru (biar foto tampil rapi)
  ...
  if (catsWithProducts.length === 0) {
    return bot.sendMessage(chatId, "Katalog belum diisi admin. Coba lagi nanti ya 🙏", {
      reply_markup: kb.catalogProductsKeyboard(),
    });
  }

  return bot.sendMessage(chatId, "📋 *Katalog & Pricelist*\n\nTap nama produk untuk lihat detailnya:", {
    parse_mode: "Markdown",
    reply_markup: kb.catalogProductsKeyboard(),
  });
}

// Dipanggil saat customer tap salah satu tombol nama produk di halaman
// Katalog & Pricelist. Menampilkan halaman baru berisi deskripsi produk
// (dari catatan yang diisi admin) beserta harganya, dengan tombol
// "Order Sekarang" (lanjut ke alur konfirmasi pesanan yang sudah ada) dan
// "Kembali" (balik ke daftar tombol nama produk).
async function showCatalogProductDetail(bot, chatId, messageId, catId, productId) {
  ...
  await bot.editMessageText(text, {
    ...
    message_id: messageId,
    parse_mode: "Markdown",
    reply_markup: kb.catalogProductDetailKeyboard(catId, productId),
  }).catch(() => {
