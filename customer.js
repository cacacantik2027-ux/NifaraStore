...
async function forwardCustomerMessageToAdmin(bot, msg, user) {
  ...
  try {
    const forwarded = await bot.forwardMessage(adminGroupId, msg.chat.id, msg.message_id);
    rememberRelay(forwarded.message_id, msg.chat.id, user.id, username);

    // Kirim juga pesan info kecil (sekali per beberapa saat cukup berisik
    // kalau tiap pesan, jadi cukup tandai forward-nya saja) — supaya kalau
    // admin scroll ke atas, jelas ini dari siapa. Info dilekatkan sebagai
    // reply ke pesan forward biar rapi, dan reply admin ke pesan info ini
    // pun tetap ke-detect balik ke customer yang sama.
    ...
    console.warn("Gagal forward pesan customer ke grup admin:", e.message);
    return false;
  }
}
