...
// Halaman "Katalog & Pricelist": deskripsi singkat (opsional, diatur admin
// lewat Panel Admin → Kelola Katalog & Pricelist) lalu daftar NAMA PRODUK
// sebagai tombol. Tap salah satu produk -> showProductInfo() (deskripsi +
// foto khusus produk itu, dengan tombol Order Sekarang & Kembali).
async function showCatalogList(bot, chatId, messageId) {
  ...
  const intro = (catalogInfo && catalogInfo.description) ||
    "Cek katalog & pricelist produk kami di bawah ini.";
  const text = hasProducts
    ? `${intro}\n\nTap salah satu produk untuk lihat detail & pesan:`
    : `${intro}\n\nKatalog belum diisi admin. Coba lagi nanti ya 🙏`;

  // Hapus pesan menu lama, kirim pesan baru (biar foto tampil rapi kalau ada)
  await bot.deleteMessage(chatId, messageId).catch(() => {});

  if (catalogInfo && catalogInfo.photoFileId) {
    return bot.sendPhoto(chatId, catalogInfo.photoFileId, {
      caption: text,
      parse_mode: "Markdown",
...
// Halaman detail 1 produk: deskripsi + foto khusus produk ini (diatur admin
// lewat Panel Admin → Kelola Kategori & Produk → pilih produk).
async function showProductInfo(bot, chatId, messageId, catId, prodId) {
  const { categories } = db.get();
  const cat = categories.find((c) => c.id === catId);
  const product = cat && cat.products.find((p) => p.id === prodId);
  if (!product) return;
  ...
  if (product.photoFileId) {
    return bot.sendPhoto(chatId, product.photoFileId, {
      caption: text,
      parse_mode: "Markdown",
      reply_markup: kb.productInfoKeyboard(catId, prodId),
    });
  }
  return bot.sendMessage(chatId, text, {
    parse_mode: "Markdown",
...
// Dipanggil saat customer tap "Order Sekarang" di halaman detail produk
// (bukan alur checkout otomatis via QRIS — ini langsung membuka live chat
// dengan admin, dengan konteks produk yang ditanyakan sudah dilampirkan).
async function startLiveChatOrder(bot, chatId, messageId, catId, prodId, user) {
  ...
  const text =
    ...
    `Harga: *${db.formatRupiah(product.price)}*\n\n` +
    `Silakan tulis pesanmu di sini (jumlah, catatan, dll), admin akan segera membalas.`;
  ...
  const sentToGroup = await bot.sendMessage(adminGroupId, groupText, { parse_mode: "Markdown" })
    .catch((e) => { console.warn("Gagal kirim notifikasi live chat order ke grup admin:", e.message); return null; });
  if (sentToGroup) {
