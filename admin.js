...
function addProduct(catId, name, price, note) {
  db.update((s) => {
    ...
    if (cat) cat.products.push({ id: "p_" + Date.now(), name, price: Number(price), note: note || "", description: "", photoFileId: null });
  });
}

// Halaman detail produk di admin sekarang juga mengelola deskripsi & foto
// khusus produk ini — ini yang tampil ke customer di halaman "Katalog &
// Pricelist" saat mereka tap nama produk (lihat customer.js:showProductInfo).
async function showProductDetailAdmin(bot, chatId, messageId, catId, prodId) {
  ...
  const text =
    ...
    `Catatan: ${prod.note || "-"}\n` +
    `Foto: ${prod.photoFileId ? "✅ sudah diatur" : "❌ belum ada (opsional)"}\n\n` +
    `Deskripsi (tampil ke customer):\n${prod.description || "-"}`;
  const rows = [
    [kb.btn("💰 Ubah Harga", { callback_data: `admin:prod_price:${catId}:${prodId}` })],
    [kb.btn("📝 Ubah Deskripsi", { callback_data: `admin:prod_desc:${catId}:${prodId}` })],
    [kb.btn(prod.photoFileId ? "🖼 Ganti Foto" : "🖼 Tambah Foto", { callback_data: `admin:prod_img:${catId}:${prodId}` })],
  ];
  if (prod.photoFileId) {
    rows.push([kb.btn("🗑 Hapus Foto", { callback_data: `admin:prod_imgdel:${catId}:${prodId}` })]);
  }
  rows.push([kb.btn("🗑 Hapus Produk", { callback_data: `admin:prod_del:${catId}:${prodId}` }, "danger")]);
  rows.push(kb.backButton(`admin:cat:${catId}`));
...
async function askProductDescription(bot, chatId, userId, messageId, catId, prodId) {
  setSession(userId, { step: "await_product_description", catId, prodId });
  await bot.editMessageText(
    "📝 Kirim deskripsi baru untuk produk ini (tampil ke customer saat mereka tap nama produk di katalog). Bisa pakai *Markdown*.\n\nKetik /batal untuk membatalkan.",
    { chat_id: chatId, message_id: messageId, parse_mode: "Markdown" }
  ).catch(() => {});
}

function updateProductDescription(catId, prodId, text) {
  db.update((s) => {
    const cat = s.categories.find((c) => c.id === catId);
    const prod = cat && cat.products.find((p) => p.id === prodId);
    if (prod) prod.description = text;
  });
}

async function askProductPhoto(bot, chatId, userId, messageId, catId, prodId) {
  setSession(userId, { step: "await_product_photo", catId, prodId });
  await bot.editMessageText(
    "🖼 Kirim foto untuk produk ini sekarang.\n\nKetik /batal untuk membatalkan.",
    { chat_id: chatId, message_id: messageId }
  ).catch(() => {});
}

function setProductPhoto(catId, prodId, fileId) {
  db.update((s) => {
    const cat = s.categories.find((c) => c.id === catId);
    const prod = cat && cat.products.find((p) => p.id === prodId);
    if (prod) prod.photoFileId = fileId;
  });
}

function deleteProductPhoto(catId, prodId) {
  db.update((s) => {
    const cat = s.categories.find((c) => c.id === catId);
    const prod = cat && cat.products.find((p) => p.id === prodId);
    if (prod) prod.photoFileId = null;
  });
}
...
module.exports = {
  ...
  askProductDescription, updateProductDescription,
  askProductPhoto, setProductPhoto, deleteProductPhoto,
